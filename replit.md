...
- **Solomon Court Search Bot:** Telegram bot (@solomon_court_ua_bot) for searching Ukrainian Supreme Court cassation decisions via court-search-agent proxy, with Claude AI query parsing and one-sentence summaries. Phone-based whitelist auth, search history, inline feedback (like/dislike) per result card, feedback analytics (`/law/analytics/data`), FastAPI `/law` endpoints. Migrations 017, 019 (solomon_feedback), 048 (solomon_scorer_log). Layer 1: parser prompt fixes — "рішення" colloquial trap eliminated, ухвали excluded by default, 4 worked examples added. Layer 2: post-fetch Haiku scorer fetches 15 candidates, scores relevance (0-10) + substantive/procedural flag, returns top N scoring ≥6 that are substantive; graceful degradation if scorer fails; exclusions logged to `solomon_scorer_log` for future classifier tuning.
