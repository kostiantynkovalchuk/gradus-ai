...
- ГОРІЛКА ≥25%: рахувати Hot Spices, Lemon, Zubrivka; НЕ рахувати ягідні лікери, бальзами → ВПЛИВАЄ на бали та passed
- ВИНО ≥40%: НЕ рахувати фруктові вина, тетрапак, вермут, box-wine, грузинські вина → ТІЛЬКИ ЗБІР ДАНИХ (Phase 1)
- КОНЬЯК ≥33% → ТІЛЬКИ ЗБІР ДАНИХ (Phase 1)
- ІГРИСТЕ ≥20% від українського АБО 4 SKU → ТІЛЬКИ ЗБІР ДАНИХ (Phase 1)
- Підполиці виключати. Порожні місця НЕ наші.
- PHASE 1: провал горілки = провал звіту [1_108608]. Провал вина/коньяку/ігристого НЕ впливає на звіт.

HORECA (тільки якщо МЧ пройдено):
- ГОРІЛКА ≥25% від вітчизняних → ВПЛИВАЄ на бали та passed
- ВИНО/КОНЬЯК ≥25% від вітчизняних (джерело: тільки меню!) → ТІЛЬКИ ЗБІР ДАНИХ (Phase 1)
...
ПІДРАХУНОК БАЛІВ (PHASE 1 — тільки горілка):
Старт 100. Кожна помилка: -10. auto_fail: -25. Відсутність огляду: -20.
Частка полиці: ГОРІЛКА <25% → -15 балів.
ВИНО, КОНЬЯК, ІГРИСТЕ → збирай дані, але НЕ знімай бали (info_only).

passed = score≥70 І кількість помилок горілки ≤2 І немає auto_fail.
Провал частки вина/коньяку/ігристого НЕ впливає на passed.

Додатково додай в JSON:
"phase": 1,
"scored_categories": ["vodka"],
"info_only_categories": ["wine", "cognac", "sparkling"]"""
