...
- ВАЖЛИВО: на касаційному рівні (ins_type=3) вироки не виносяться → якщо "вирок" + касація → vr_type "2"

Форма судочинства:
- податков*/ДПС/перевірка/ППР/нереальність/маркетинг витрати/КАС/КАСУ → cs_type "4"
- вирок/обвинувачення/злочин/КК України → cs_type "2"
- договір/стягнення/банкрутство/борг/ГК → cs_type "3"
- аліменти/спадщина/нерухомість/ЦК → cs_type "1"
...
Номер справи:
- Номер справи має СТРОГО відповідати формату з двома слешами: XXX/XXXXX/XX (наприклад 910/12568/24, 380/25114/23)
- Тільки якщо є повний номер з двома "/" → case_number: "номер", search_text порожньо
- Окремі числа (324, 185) — це НЕ номери справ, це статті законів або інші числа
- "справа 910/12568/24" → case_number: "910/12568/24"
- "знайди 380/25114/23" → case_number: "380/25114/23"
...
- Якщо є стаття + ключові слова → search_text: "стаття N ключове_слово" (макс 5 слів загалом)

Кількість результатів:
- "покажи 10", "дай 3 приклади", "топ 7" → limit: N
- "більше результатів" або "більше" без числа → limit: 10
- За замовчуванням → limit: 5
- Максимум: 20

search_text:
- Якщо є номер справи → search_text порожньо
- Якщо є конкретні фільтри (тип + форма + дати) → search_text може бути порожнім
...
def search_decisions(params: dict) -> list:
    ...
    if params.get("case_number"):
        ...
        if resp.status_code == 200:
            ...
            return _build_results(judgments, limit)
        return []
...
def _build_results(judgments: list, limit: int) -> list:
