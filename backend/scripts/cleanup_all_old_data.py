...
def cleanup_all():
    ...
    print(f"📊 Current total: {stats['total_vector_count']} vectors")
    ...
    for match in results['matches']:
        ...
        if 'greenday' in text or metadata.get('brand') == 'GREENDAY':
            if 'GREENDAY_PRODUCT_MANUAL' in vector_id:
                ...
                print(f"✅ KEEP: {vector_id[:60]}")
            else:
                greenday_old.append(vector_id)
                to_delete.append(vector_id)
                print(f"❌ DELETE: {vector_id[:60]}")
    
    print(f"\nGREENDAY Summary:")
    print(f"   ✅ Keep: {len(greenday_manual)} manual PRODUCT chunks")
    print(f"   ❌ Delete: {len(greenday_old)} old chunks")
    ...
    if len(bb_chunks) > 3:
        print(f"\n✅ Keeping first 3 Best Brands chunks")
        for chunk in bb_chunks[:3]:
            print(f"   KEEP: {chunk['id'][:60]}")
        
        print(f"\n❌ Deleting {len(bb_chunks) - 3} excessive Best Brands chunks")
        for chunk in bb_chunks[3:]:
    ...
    if to_delete:
        print(f"\n⚠️  This will delete {len(to_delete)} vectors")
        ...
        if confirm.lower() == 'yes':
            ...
            stats = index.describe_index_stats()
            print(f"📊 New total: {stats['total_vector_count']} vectors")
        else:
