...
async def alert_hr_team_identified(
    ...
    """
    Send an identified risk alert to HR_ALERT_CHAT_ID with HR action buttons.
    ...
    try:
        if callback_data.startswith('hr_menu:'):
            ...
            if menu_id == 'main':
                ...
                if is_english_pilot(chat_id):
                    _training_rows.append([{
                        "text": "🗣️ Розмовна англійська",
                        "url": "https://t.me/sara_tutor_bot?start=maya_hr",
                    }])
                _training_rows.append([{"text": "🏠 Головне меню", "callback_data": "hr_menu:main"}])
                training_keyboard = {"inline_keyboard": _training_rows}
                training_msg = (
                    "📚 *Навчальні матеріали*\n\n"
            ...
            action_keyboard = {
                "inline_keyboard": [
                    [
                        ...
                        {"text": "📢 Robota.ua", "callback_data": f"hunt_action_post_{vacancy_id}"},
                    ],
                    [
                        {"text": "🔍+📢 Пошук + Robota.ua", "callback_data": f"hunt_action_both_{vacancy_id}"},
