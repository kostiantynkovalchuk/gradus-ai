...
async def lifespan(app: FastAPI):
    ...
    if RENDER_URL and os.getenv("SOLOMON_BOT_TOKEN"):
        try:
            from solomon_bot import setup_solomon_webhook
            await setup_solomon_webhook(RENDER_URL)
            logger.info("⚖️ Solomon webhook registered")
        except Exception as e:
            logger.error(f"Solomon webhook setup failed: {e}")
    else:
        ...
        try:
            ...
            async with httpx.AsyncClient() as hclient:
                resp = await hclient.post(
                    ...
                    json={"url": f"{RENDER_URL}/api/telegram/webhook/gradus"}
                )
        ...
        if "facebook" in [p.lower() for p in request.platforms]:
            ...
            if content.extra_metadata and content.extra_metadata.get('fb_post_id'):
                ...
                post_data = {
                    ...
                    'article_id': content.id,  # For public image URL fallback (Priority 2)
                    'image_url': content.image_url,
                    'local_image_path': content.local_image_path,
                    'image_data': content.image_data  # Binary from database (Priority 1)
                }
